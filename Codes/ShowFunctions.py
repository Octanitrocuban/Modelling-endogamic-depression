...
#=============================================================================
def plotEvolGen(ArrOfMut, FgSz=(16, 6), Leg=False, marge=0.01):
	"""
...
def plotEvolConsang(ArrOfPop, FgSz=(16, 6), Leg=False, marge=0.01):
	"""
	...
	"""
	Nnb = int(ArrOfPop[-1, 4])
	ampli = Nnb
	lower = -ampli*marge
	upper = Nnb+ampli*marge
	consang = ArrOfPop[:, 5].astype(float)
	scores = np.zeros(Nnb+1)
	for i in range(Nnb+1):
		scores[i] = np.mean(consang[ArrOfPop[:, 4] == str(i)])
...
def EvolLenPopShow(ArrLenPop, ArrSumPop, FgSz=(10, 6), marge=0.01):
	"""
	...
	ampli = len(ArrLenPop)
	lower = -ampli*marge
	upper = len(ArrLenPop)-1+ampli*marge
	
	plt.figure(figsize=FgSz)
	plt.title("Evolution of the population", fontsize=22)
	plt.grid(True, zorder=1)
	plt.plot(ArrLenPop, '.-', label="By generation", zorder=2)
	...
	plt.grid(True, zorder=1)
	plt.plot(ArrSumPop, '.-', label="Cumulated", zorder=2)
	plt.legend(fontsize=15, loc='upper left')
	plt.xlabel("Generation", fontsize=15)
...
def GenealogicTree(ArrOfPop, Figsize=(16, 10), STitle=20, marge=0.02):
	"""
	...
	ampli = len(TreeCut)-1
	lower = -1-ampli*marge
	upper = len(TreeCut)-1+ampli*marge
...
def InformativLinearTree(ArrOfPop, Figsize=(18, 14), STitl=22, Sscat=30,
	...
	PercMut = np.zeros(ArrOfPop[:, 6:].shape)
	PercMut[ArrOfPop[:, 6:] == 'aa'] = 2
	PercMut[ArrOfPop[:, 6:] == 'Aa'] = 1
	PercMut[ArrOfPop[:, 6:] == 'aA'] = 1
	PercMut = np.sum(PercMut, axis=1)/(len(ArrOfPop[0, 6:])*2)
...
def ConsangLinearTree(ArrOfPop, Figsize=(18, 14), STitl=22, Sscat=30,
					  Cmap='jet', marge=0.02):
	"""
	This fuction used to show the genealogical link between the peoples
	created during the simulation, and their mutation rate (=number of mutated
	...
	for i in range(Tree[-1, 1]+1):
		TreeCut.append(Tree[Tree[:, 1] == i])
		Gend.append(ArrOfPop[Tree[:, 1] == i , 1])
		Mutcut.append(ArrOfPop[Tree[:, 1] == i, 5].astype(float))
	...
	plt.figure(figsize=Figsize)
	plt.title("Genealogical tree & consanguinity", fontsize=STitl)
	plt.plot(-1, cent, 'k*', markersize=15, zorder=3, label='LUCA')
	plt.plot(-4, cent, 'ko', markersize=10, zorder=2, label='Woman')
	plt.plot(-4, cent, 'k^', markersize=10, zorder=1, label='Man')
	for i in range(len(TreeCut)):
